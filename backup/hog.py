# Names: Juan Pinzon    cs61a-anj   StudentID:23632316
#        Jasbir Kaur    cs61a-ed    StudentID:18926887
# Taking turns
...
def roll_dice(num_rolls, dice=six_sided):
    ...
    dice_value_one = False                          #Shows if any dice rolls 1
    for k in range(0, num_rolls):
        dice_value = dice()
        if dice_value == 1:                         #Pig Out rule
            dice_value_one = True
        else:                                       #No Pig Out Rule --> then add dice value to the score  
            score += dice_value        
    if dice_value_one:
        score = 1                                   #If any dice rolls 1 then score=1
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while score < goal and opponent_score < goal:
        ...
        if score == (2 * opponent_score) or opponent_score == (2 * score):                #Swine Swap - swap scores            
...
def swap_strategy(score, opponent_score):
    ...
    """
    "*** YOUR CODE HERE ***"
    play_zero = score + take_turn(0, opponent_score)                                    #Score after player rolls 0
    if play_zero == (2 * opponent_score) or opponent_score == (2 * play_zero):          #Detect a possible swap
        if play_zero < opponent_score:													#beneficial swap
            return 0
        else:                                                                           #harmful swap
            return BASELINE_NUM_ROLLS
    else:                                                                               #If rolling 0 would not cause a swap
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    My Strategy is based in roll 0 dice
	First I compare the bacon strategy as well the swine swap strategy with roll 0 dice and return the greatest of those values. 
	I follow the hints, like vary my rolls based on whether you will be rolling four-sided or six- sided dice, or change the number of rolls if I am leading or loosing,
    Also I implemented the strategy to leave my opponent with four-sided dice more often. but nothin works. Maybe We follow teh wrong direction, but we try the most. I feel frustrated
	Then I check if I am close to the goal, and if I am leading ot loosing
	The truth We spent a lot of time in the strategy but We got stuck here.
    """
    "*** YOUR CODE HERE ***"    
    base = take_turn(0, opponent_score, select_dice(score, opponent_score))                                     #Base strategy is when I play zero dice
    if base <= take_turn(bacon_strategy(score, opponent_score),opponent_score) :
        return bacon_strategy(score, opponent_score)
    
    elif base <= take_turn(swap_strategy(score, opponent_score),opponent_score):
        return swap_strategy(score, opponent_score)
    
    #elif select_dice(score, opponent_score) == four_sided:
        #return max_scoring_num_rolls(four_sided)
    #else:
        #return max_scoring_num_rolls(six_sided)
